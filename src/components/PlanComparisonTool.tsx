...
export default function PlanComparisonTool() {
    ...
    return (
        <div className="min-h-screen bg-white">
                ...
                <div className="container mx-auto px-2 py-3 sm:px-4 sm:py-4">
                    <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-2 sm:gap-0">
                        <div className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-2">
                            <svg viewBox="0 0 54 54" className="h-8 w-8 text-white mb-1 sm:mb-0" fill="currentColor">
                            ...
                            <h1 className="text-lg sm:text-xl font-bold text-center sm:text-left">Slack Plan Comparison</h1>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <main className="container mx-auto px-2 py-4 sm:px-4 sm:py-8">
                <div className="mb-6 sm:mb-8 text-center">
                    <h2 className="text-lg sm:text-2xl font-bold text-[#4A154B]">Find the right Slack plan for your team</h2>
                    <p className="mt-1 sm:mt-2 text-sm sm:text-base text-gray-600">Compare plans to see what features you'll gain by upgrading</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-12">
                    ...
                    <div className="md:col-span-4">
                        <div className="rounded-lg bg-gray-50 p-4 sm:p-6 shadow-sm">
                            <h3 className="mb-3 sm:mb-4 text-base sm:text-lg font-semibold text-[#36C5F0]">Select Options</h3>

                            <form onSubmit={handleSubmit}>
                                <div className="mb-3 sm:mb-4">
                                    <label htmlFor="currentPlan" className="mb-1 block font-medium text-[#36C5F0] text-sm sm:text-base">
                                        Current Plan
                                    </label>
                                    <div className="relative">
                                        <select
                                            ...
                                            className="w-full appearance-none rounded-md border border-gray-300 bg-white px-3 py-2 pr-10 text-gray-900 text-sm sm:text-base shadow-sm focus:border-[#36C5F0] focus:outline-none focus:ring-2 focus:ring-[#36C5F0]/50"
                                            disabled={isLoading}
                                        >
                                ...
                                <div className="mb-3 sm:mb-4">
                                    <label htmlFor="futurePlan" className="mb-1 block font-medium text-[#36C5F0] text-sm sm:text-base">
                                        Future Plan
                                    </label>
                                    <div className="relative">
                                        <select
                                ...
                                <div className="mb-5 sm:mb-6">
                                    <label htmlFor="lineOfBusiness" className="mb-1 block font-medium text-[#36C5F0] text-sm sm:text-base">
                                        Line of Business
                                    </label>
                                    <div className="relative">
                                        <select
                                ...
                                <button
                                    type="submit"
                                    className="w-full rounded-full bg-[#36C5F0] px-4 py-2 font-medium text-white text-base sm:text-base shadow-sm transition-colors hover:bg-[#2EAAD3] focus:outline-none focus:ring-2 focus:ring-[#36C5F0]/50 focus:ring-offset-2 disabled:opacity-70 min-h-[44px]"
                                    disabled={isLoading}
                                >
                                    {isLoading ? (
                    ...
                    <div className="md:col-span-8 mt-6 md:mt-0">
                        {!isSubmitted ? (
                            <div className="flex h-full items-center justify-center rounded-lg bg-gray-50 p-6 sm:p-8 text-center">
                                <div>
                                    <div className="mx-auto mb-4 flex h-12 w-12 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-[#ECF4FB]">
                                        <ArrowRight className="h-6 w-6 sm:h-8 sm:w-8 text-[#36C5F0]" />
                                    </div>
                                    <h3 className="text-base sm:text-lg font-medium text-gray-900">Select plans to compare</h3>
              ...
              <div className="rounded-lg bg-white shadow-sm">
                {/* Tabs/Info Bar */}
                <div className="flex flex-col sm:flex-row items-center justify-between border-b px-4 sm:px-6 py-2 sm:py-3 gap-2 sm:gap-0">
                  <div className="font-medium text-[#36C5F0] text-base sm:text-lg">Upgrade Features</div>
                  {isSubmitted && submittedLineOfBusiness && (
                    <div className="flex items-center rounded-full bg-gray-100 px-2 sm:px-3 py-1 text-xs sm:text-sm">
                ...
                <div className="p-4 sm:p-6">
                  <div className="mb-3 sm:mb-4 flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                    <div className="flex items-center space-x-2">
                      <div className="rounded-md bg-gray-100 px-2 py-1 text-xs sm:text-sm font-medium text-gray-800">
                        {planOptions.find((option) => option.value === currentPlan)?.label}
                      </div>
                      <ArrowRight className="h-4 w-4 text-gray-400" />
                      <div className="rounded-md bg-[#4A154B] px-2 py-1 text-xs sm:text-sm font-medium text-white">
                        {planOptions.find((option) => option.value === futurePlan)?.label}
                      </div>
                    </div>
                  </div>

                  <h3 className="mb-3 sm:mb-4 text-lg sm:text-xl font-semibold text-[#4A154B]">Features you'll gain by upgrading</h3>
                  ...
                  {upgradeFeatures.length === 0 ? (
                    ...
                    <div className="space-y-6 sm:space-y-8">
                      {Object.entries(categorizedFeatures).map(([category, features]) => (
                        <div key={category}>
                          <h4 className="mb-2 sm:mb-3 text-base sm:text-lg font-medium" style={{ color: getSectionColor(category) }}>
                            {category}
                          </h4>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            {features.map((feature) => {
                              ...
                              return (
                                <div
                                  key={feature}
                                  className={`group relative rounded-lg border bg-white p-3 sm:p-4 shadow-sm transition-all hover:shadow-md ${
                                    hasPainPoint ? "border-l-4 border-l-[#E01E5A] border-gray-200" : "border-gray-200"
                                  }`}
                                >
                                  <div className="mb-1 sm:mb-2 flex items-start">
                                    <div className="mr-2 sm:mr-3 mt-0.5 flex-shrink-0">{getFeatureIcon(feature)}</div>
                                    <div className="w-full">
                                      <h4 className="font-semibold text-[#4A154B] text-sm sm:text-base">{feature}</h4>
                                      ...
                                      {hasPainPoint && (
                                        <div className="mt-2 sm:mt-3 border-t border-dashed border-gray-200 pt-2 sm:pt-3">
                                          <div className="flex items-center">
                                            <div className="mr-2 h-2 w-2 rounded-full bg-[#E01E5A]"></div>
                                            <h5 className="text-xs sm:text-sm font-medium text-[#E01E5A]">
                                              {lobOptions.find((option) => option.value === submittedLineOfBusiness)?.label} Pain
                                              Point
                                            </h5>
                                          </div>
                                          <p className="mt-1 text-xs sm:text-sm text-gray-700">{painPoints[feature]}</p>
                                        </div>
                                      )}
                                    </div>
                                  </div>
        ...
        <footer className="bg-gray-50 py-6 sm:py-8">
            <div className="container mx-auto px-2 sm:px-4 text-center text-xs sm:text-sm text-gray-500">
