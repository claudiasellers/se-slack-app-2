...
export default function PlanComparisonTool() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      try {
        ...
        // Set the first category as active tab if there are categories
  ...
  // Render feature cards for the active tab
  const renderFeatureCards = (category: string) => {
    if (!categorizedFeatures[category]) return null
                    ...
                    <div className="mt-3 border-t border-dashed border-gray-200 pt-3">
                      <div className="flex items-center">
                        ...
                        <h5 className="text-sm font-medium text-[#E01E5A]">
                          {lobOptions.find((option) => option.value === submittedLineOfBusiness)?.label} Pain Point
                        </h5>
                      </div>
                ...
                <div className="p-6">
                  ...
                  {upgradeFeatures.length === 0 ? (
                    ...
                    <div className="space-y-6">
                          ...
                          {Object.keys(categorizedFeatures).map((category) => (
                            <button
                              ...
                              className={`whitespace-nowrap px-4 py-2 text-sm font-medium transition-colors ${
                                activeTab === category
                                  ? `border-b-2 text-${getSectionColor(category)}`
                                  : "text-gray-600 hover:text-gray-900"
                              }`}
                              style={{
                                borderColor: activeTab === category ? getSectionColor(category) : "transparent",
                                color: activeTab === category ? getSectionColor(category) : undefined,
                              }}
                            >
                              {category}
                            </button>
                      ...
                      {/* Active Tab Content */}
                      <div className="mt-4">{activeTab && renderFeatureCards(activeTab)}</div>
                    </div>
                  )}
                </div>
              </div>
